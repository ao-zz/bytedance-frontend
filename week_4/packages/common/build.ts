...
export async function copyNpmToTt(npmPath) {
...
export async function buildSinglePage(page, tarnsform) {
    ...
    const outputPageTTMLPath = `${outPageDirPath}.ttml`
    const outputPageTTSSPath = `${outPageDirPath}.ttss`
...
Page(require('${relativeAppPath}').createPage(${result.className}))
    ...
    fse.writeFileSync(outputPageTTMLPath, result.ttml)
    console.log(`输出文件：${outputDir}/${page}.ttml`.info)
    fse.writeFileSync(outputPageTTSSPath, result.style)
    console.log(`输出文件：${outputDir}/${page}.ttss`.info)
}
