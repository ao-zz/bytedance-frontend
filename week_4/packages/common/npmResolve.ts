...
function getTtNpmPath(path) {
    return `/npm${getNpmRelativePath(path)}`
}

function getTtRelativePath(path, filePath) {
    const rpath = npath.relative(getDirPath(filePath), resloveTtNpmPath(path))
    return judgeLibPath(rpath) ? `./${rpath}` : rpath
}

function resloveTtNpmPath(path) {
    return npath.join(outputDir, getTtNpmPath(path))
}
...
async function copyNpmToTT(filePath, npmPath, isRoot = false) {
        ...
        CallExpression(path) {
            if (t.isIdentifier(path.node.callee, {name: 'require'})) {
                ...
                if (judgeLibPath(sourcePath)) {
                    ...
                    copyNpmToTT(mainPath, npmPath)
                    path.node.arguments[0].value = getTtRelativePath(mainPath, resloveTtNpmPath(filePath))
                } else if (npmPath) {
                    const _filePath = npath.resolve(npmPath, addJsFile(sourcePath))
                    copyNpmToTT(_filePath, npmPath)
                }
            }
    ...
    if (isRoot) {
        fileContent.forEach(async (value, filePath) => {
            const _filePath = resloveTtNpmPath(filePath)
...
function parseNpm(sourcePath, filePath) {
    ...
    copyNpmToTT(mainPath, npmPath, true)
    return getTtRelativePath(mainPath, filePath)
}
